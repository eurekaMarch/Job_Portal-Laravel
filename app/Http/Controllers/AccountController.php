...
class AccountController extends Controller
{
    ...
    {
        $id = Auth::user()->id;
        $jobs = Job::where('user_id', $id)->with(['jobType', 'applications'])->orderBy('created_at', 'DESC')->paginate(10);
    ...
    {
        $id = Auth::user()->id;
        $jobApplications = JobApplication::where('user_id', $id)->with(['job', 'job.jobType', 'job.applications'])->paginate(10);
    ...
    function removeSavedJobs(Request $request)
    {
        $savedJobs = SavedJob::where([
            'user_id' => Auth::user()->id,
            'id' => $request->id,
    ...
    function processForgotPassword(Request $request)
    {
        $validator = Validator::make(
            $request->all(),
            [
        ...
        return redirect()->route('forgotPassword')->with('success', 'Reset password email has been sent to your inbox.');
    }
